...
namespace Recolor.Systems.SelectedInfoPanel
{
    ...
    public partial class SIPColorFieldsSystem : ExtendedInfoSectionBase
    {
        ...
        private void HandleScopeAndButtonStates()
        {
            ...
            {
                if (EntityManager.TryGetBuffer(m_CurrentEntity, isReadOnly: true, out DynamicBuffer<Game.Rendering.CustomMeshColor> customMeshColors) &&
                    EntityManager.IsComponentEnabled<Game.Rendering.CustomMeshColor>(m_CurrentEntity) &&
        ...
        private ColorSet ChangeColor(int channel, UnityEngine.Color color)
        {
            ...
            // Color Variations
                    ...
                    (!EntityManager.IsComponentEnabled<Game.Rendering.CustomMeshColor>(m_CurrentEntity) ||
                    (EntityManager.TryGetBuffer<Game.Rendering.CustomMeshColor>(m_CurrentEntity, isReadOnly: true, out DynamicBuffer<Game.Rendering.CustomMeshColor> customMeshColors) &&
                     customMeshColors.Length == 0)))
            {
                if (!EntityManager.TryGetBuffer(m_CurrentPrefabEntity, isReadOnly: true, out DynamicBuffer<SubMesh> subMeshBuffer))
                {
        ...
        private ColorSet ChangeSingleInstanceColorChannel(int channel, UnityEngine.Color color, Entity entity, EntityCommandBuffer buffer)
        {
            ...
            if (!EntityManager.TryGetBuffer(entity, isReadOnly: false, out DynamicBuffer<Game.Rendering.CustomMeshColor> customMeshColors) ||
                customMeshColors.Length != m_SubMeshData.Value.SubMeshLength)
            {
                ...
                if (!EntityManager.HasBuffer<MeshColorRecord>(entity))
                {
                    ...
                    for (int i = 0; i < m_SubMeshData.Value.SubMeshLength; i++)
                    {
                        if (meshColorBuffer.Length > i)
                        {
                            meshColorRecordBuffer.Add(new MeshColorRecord(meshColorBuffer[i]));
                        }
                        else
                        {
                            meshColorRecordBuffer.Add(new MeshColorRecord(meshColorBuffer[0]));
                        }
                    }
                }
            }

            if (!EntityManager.TryGetBuffer(entity, isReadOnly: false, out DynamicBuffer<Game.Rendering.CustomMeshColor> customMeshColorBuffer))
            {
                return default;
            }
            ...
            if (m_SubMeshData.Value.SubMeshLength == 1)
            {
                Game.Rendering.CustomMeshColor customMeshColor = customMeshColorBuffer[0];
                if (channel >= 0 && channel < 3)
                {
            ...
            else
            {
                ...
                for (int i = 0; i < m_SubMeshData.Value.SubMeshLength; i++)
                {
                    if (!m_SubMeshIndexes.Contains(i) &&
                        ...
                        m_Route.Value != ButtonState.On)
                    {
                        recolorCustomMeshColors.Add(new Domain.CustomMeshColor(customMeshColorBuffer[i].m_ColorSet));
                        continue;
                    }
                    ...
                    recolorCustomMeshColors.Add(new Domain.CustomMeshColor(customMeshColor.m_ColorSet));
                    colorSet = customMeshColor.m_ColorSet;
                }
            }
        ...
        private void ResetColor(int channel)
        {
            ...
            // Single Instanc e
            if (EntityManager.TryGetBuffer(m_CurrentEntity, isReadOnly: true, out DynamicBuffer<Game.Rendering.CustomMeshColor> customMeshColor) &&
                EntityManager.IsComponentEnabled<Game.Rendering.CustomMeshColor>(m_CurrentEntity) &&
                customMeshColor.Length > 0)
            {
